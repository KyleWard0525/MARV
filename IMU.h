...
class IMU {
  ...
  private: 
    ...
    const uint16_t AzLowAddr = 0x40;    //  I2C address of Y-acceleration's high byte
    const uint16_t gStartAddr = 0x43;   //  I2C address to start reading acceleration values from (0x43 = GYRO_XOUT)
  ...
  public:
    ...
    double accelOffset;                 //  Acceleration offset
    ...
    { 
      ...
      if(serialBus->readByte(mpuAddr, whoAmIAddr) != 0x68)
      {
        ...
        while(!mpu->begin(MPU6050_SCALE_1000DPS, MPU6050_RANGE_4G))
        {
    ...
    void start()
    {
      // Wake up MPU
       ...
       // Configure accelerometer and gyrometer

       /*
        * Gyro and accel config registers only use bit 3 and 4 for setting the full scale
        * range
        */
        this->gyroConfig = 0x10;  // 0x10 = 000[10]000 = 2 = +-1000 deg/s
        ...
        if(serialBus->writeByte(mpuAddr, gConfigAddr, gyroConfig))
        {
          ...
          Serial.println("Gyrometer configuration failed!");
        }

       // Write accelConfig to accel config register
